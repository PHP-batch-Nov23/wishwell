...
export class DetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      this.serviceBackend.getAllCampaignDonations(this.campaignId).then(
        data=>this.donations=data.data.reverse());
  ...
  donate(): void {
    ...
    this.serviceBackend.getUser().then(data=>{
      ...
      if(this.donationAmount>data.data.balance){
        ...
        this.serviceBackend.createDonationEntry({"donor_id":data.data.id,"campaign_id":this.campaignId,"amount":this.donationAmount}).then(data=>{
